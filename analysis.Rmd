...
binary_variables <- c("HI", "gender", "born", "perceptive")
...
contrast <- cbind (c(-0.5, +0.5))   # f = -0.5, m = +0.5
colnames (contrast) <- c("-f+m")
contrasts (data$gender) <- contrast
contrasts (data$gender)

# Binary variable `born`
data$born <- as.factor(data$born)
contrast <- cbind (c(+0.5, -0.5))   # born = -0.5, not_born = +0.5
colnames (contrast) <- c("-born+not_born")
contrasts (data$born) <- contrast
contrasts (data$born)

# Variable perceptive can take three values
data$perceptive <- as.factor(data$perceptive)
contrasts <- cbind (c(-1/3, +2/3, -1/3), c(-0.5, 0, +0.5))   # alphabetical order: conductive, conductive_perceptive, perceptive
colnames (contrasts) <- c("-conductiveperceptive+conductive_perceptive", "-conductive+perceptive")
contrasts (data$perceptive) <- contrasts
contrasts(data$perceptive)
```
...
numeric_variables_for_cor_bus <- c(
  "storylength_bus", 
  "MLU_bus", 
  "mazes_bus", 
  "ungrammatical_bus", 
  ...
  numeric_variables_both
)

numeric_variables_for_cor_frog <- c(
  "storylength_frog", 
  "MLU_frog", 
  "mazes_frog", 
  "ungrammatical_frog", 
...
make_corrplot(data[,numeric_variables_for_cor_bus], "correlation_matrix_bus", cex=0.6, ncex=0.4)
make_corrplot(data[,numeric_variables_for_cor_frog], "correlation_matrix_frog", cex=0.6, ncex=0.4)
```

The p-values for the correlations are shown as well.
...
Here, the correlations of several variables regarding phonological processing,
...
make_corrplot(data[c(dependent_variables_frog, independent_variables_cognition)], "The Frog Story, NWRS & BRIEF (total)")
...
```{r}
make_corrplot(data_HI[c(dependent_variables_frog, independent_variables_cognition)], "The Frog Story, NWRS & BRIEF (HI)", cex=0.7, ncex=0.7)
...
```{r}
make_corrplot(data_TD[c(dependent_variables_frog, independent_variables_cognition)], "The Frog Story, NWRS & BRIEF (TD)")
...
make_corrplot(data[c(dependent_variables_bus, independent_variables_cognition)], "The Bus Story, NWRS & BRIEF (total)")
...
```{r}
make_corrplot(data_HI[c(dependent_variables_bus, independent_variables_cognition)], "The Bus Story, NWRS & BRIEF (HI)")
...
```{r}
make_corrplot(data_TD[c(dependent_variables_bus, independent_variables_cognition)], "The Bus Story, NWRS & BRIEF (TD)", cex=0.7, ncex=0.7)
